	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				$tbody.find("a.btnDel").click(function(){
					...
					if ($btnDel.is("[href^=javascript:]")) {
					    var rowSum = 0;
					    var $rowSum = $('input[name="dwz_rowSum"]');
					    try { rowSum = parseInt($rowSum.val()) } catch (e) { }
					    rowSum = rowSum - 1;
					    $rowSum.val(rowSum);
			...
			function tdHtml(field){
			    var html = '', suffix = ''; readonly = '';
			    if (field.fieldClass.indexOf("readonly") > -1) {
				...
				switch(field.type){
					...
					case 'lookup':
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							+ '<input type="text" '+ readonly +' name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" '+ ro +' size="'+field.size+'" class="'+field.fieldClass+'"/>'
							+ '<a class="btnLook" width="200" height="300" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'

...
$.fn.extend({
	tabControl: function(options, tags) {
		...
		var initTab = function(obj, index) {//初始化标签输入
		    var textHtml = "<input class='tabinput' name='" + opts.tabName + "' style='width:" + opts.tabW + "px;height:" + opts.tabH + "px;' type='text'/>";
			obj.append(textHtml);
			if (_tags[index]) {
			    var __inputobj = $("input[type='text'][name='" + opts.tabName + "']", obj).eq(index);
		    ...
		    $("input[type='text'][name='" + opts.tabName + "']:last", obj).bind("keydown blur click",
			function(event) {
				...
				if (event.keyCode == 13 || event.keyCode == 9 || event.type == "blur") {
					...
					if (value != "") {
						...
						if (!_match) {
							...
							if ($("input[type='text'][name='" + opts.tabName + "']", obj).length < opts.maxTabCount) {
								if (!inputObj.data("isModify"))
								 initTab(obj);
								else if (!$("input[type='text'][name='" + opts.tabName + "']", obj).is(":hidden")) {
									initTab(obj);
								}
							}
		...
		return this.each(function() {
			...
			jqObj.click(function() {
			    $("input[type='text'][name='"+ opts.tabName +"']", jqObj).each(function() {
					if ($(this).val() == "") {
						$(this).focus();
						return false;
					}
